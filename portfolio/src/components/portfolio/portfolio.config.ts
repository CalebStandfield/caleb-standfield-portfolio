...
export const projectSlots: ProjectData[] = [
  {
    ...
    title: "blackjack",
    summary:
      "A complete blackjack game engine built from scratch in C++ with Qt. Fully AI-driven opponents give the table life and challenge. Built with a team of six on an Agile workflow with daily scrums, it leans on clear OOP design, event handling, and finite state management.",
    tags: [
      "C++",
      "Qt Creator",
      "Full Stack",
    ...
    repositoryUrl:
      "https://github.com/CalebStandfield/full-blackjack-game-engine-with-UI",
  },
  {
    id: "pixelify",
    status: "in_progress",
    title: "Pixelify",
    summary:
      "Image-processing engine built to turn regular images into a pixel-art style, with advanced capabilities coming soon. Designed to be fast and robust. Always adding new features and improving performance.",
    tags: ["Rust",
      "System Design",
      "Full Stack",
      "UX Design",
  ...
  },
  {
    ...
    title: "learn-vim",
    summary:
      "An interactive Vim trainer built around real edits, not quizzes. Transform text and/or code in as few keystrokes as you can, with instant feedback and par scores to beat. Used to teach coworkers at Adobe the basics of Vim, but also how to use Vim with AI and Obsidian.",
    tags: [
      "JavaScript",
      "React",
      "Vim Emulation",
  ...
  },
  {
    ...
    title: "Sprite Editor",
    summary:
      "A pixel-art sprite editor and animation tool built from scratch in C++ with Qt. Draw, edit, and animate in real time with smart tools (line, fill, circle, square), each powered by custom algorithms. Layers, palettes, and animation data save to JSON, with live previews and frame-by-frame playback.",
    tags: [
      "C++",
      "Qt Creator",
      "Algorithmic Tools",
...
export const resume = {
  ...
  work: "Adobe summer internship 2026",
} as const;
...
export const systemStandalones: SystemStandaloneData[] = [
  ...
  },
  {
    ...
    width: 52,
    align: "end",
    eyebrow: "durable media",
  },
  {
    nodeId: "email_gateway",
    scene: "operations",
    stage: "contact",
    offset: 200,
    width: 50,
    align: "start",
    eyebrow: "external delivery",
  },
  {
    nodeId: "backup_vault",
    scene: "operations",
...
export const systemNodes: SystemNodeData[] = [
  ...
  node({
    id: "mobile_app",
    title: "Mobile App",
    subtitle: "Typed client",
    icon: DeviceMobile,
    kind: "client",
    language: "ts",
    code: `const res = await\n  api.media.list();\nsetItems(res.data);\nnavigate("/feed");`,
  }),
  node({
    id: "cdn",
    title: "CDN",
    subtitle: "Assets + media",
    icon: CloudArrowDown,
    kind: "edge",
    language: "nginx",
  ...
  node({
    id: "waf",
    title: "WAF",
    subtitle: "Threat filtering",
    icon: ShieldCheck,
    kind: "security",
    language: "nginx",
    code: `limit_req zone=api\n  burst=40 nodelay;\nif ($bad_bot) {\n  return 403;\n}`,
  }),
  node({
    id: "api_gateway",
    title: "API Gateway",
    subtitle: "Routing + policy",
  ...
  node({
    ...
    kind: "service",
    language: "rust",
    code: `db.insert(project)\n  .await?;\nbus.publish(\n  ProjectAdded)\n  .await?;`,
  }),
  node({
    id: "media_service",
    title: "Media Service",
    subtitle: "Upload contracts",
    icon: CloudArrowUp,
    kind: "service",
    language: "rust",
    code: `pub async fn upload(key: Key)\n  -> Result<PresignedUrl> {\n  let url = store\n    .presign_put(key).await?;\n  Ok(url)\n}`,
  }),
  node({
    id: "redis",
    title: "Redis Cache",
    subtitle: "Read-through cache",
    icon: Lightning,
    kind: "data",
    language: "rust",
    code: `cache\n  .get(key)\n  .await?\n  .or_load()`,
  ...
  node({
    ...
    kind: "data",
    language: "rust",
    code: `store.put(key, bytes)\n  .content_type(mime)\n  .send().await?;\nOk(Key::from(key))`,
  }),
  node({
    id: "event_bus",
    title: "Event Bus",
    subtitle: "Durable topics",
    icon: Broadcast,
    kind: "async",
    language: "rust",
    code: `bus.publish(Topic::MediaUploaded, Event {\n  key: media.key.clone(),\n  at: Utc::now(),\n}).await?;`,
  }),
  node({
    id: "media_worker",
    title: "Media Worker",
    subtitle: "Image pipeline",
    icon: GearSix,
    kind: "async",
    language: "rust",
    code: `let out = image\n  ::load(&src)?\n  .resize(1600)\n  .to_webp()?;\nstore.put(out)?;`,
  ...
  node({
    id: "email_gateway",
    title: "Email Gateway",
    subtitle: "External delivery",
    icon: EnvelopeSimple,
    kind: "service",
    language: "rust",
    code: `provider.send(Alert {\n  to: oncall.email,\n  incident: id,\n}).await?;`,
  }),
  node({
    id: "git_repository",
    title: "Git Repository",
    subtitle: "Source + manifests",
    icon: GitBranch,
    kind: "delivery",
    language: "yaml",
    code: `branch: main\nprotected: true\nrequire_reviews: 2\nchecks:\n  - build\n  - test`,
  ...
  node({
    id: "oci_registry",
    title: "OCI Registry",
    subtitle: "Signed images",
    icon: Package,
    kind: "delivery",
    language: "yaml",
    code: `image: profile-api\ntag: sha-7f3c9a1\ndigest: sha256:8e2...\nsigned: true\nsigner: cosign`,
  }),
  node({
...
export const trafficScenarios: TrafficScenario[] = [
  {
    ...
    steps: [
      { edgeIds: ["web-cdn"] },
      { edgeIds: ["cdn-object"] },
      { edgeIds: [], reverseEdgeIds: ["cdn-object", "web-cdn"] },
    ],
  },
  {
    id: "profile-read",
    ...
    steps: [
      { edgeIds: ["web-waf"] },
      { edgeIds: ["waf-gateway"] },
      { edgeIds: ["gateway-auth"] },
      { edgeIds: [], reverseEdgeIds: ["gateway-auth"] },
      { edgeIds: ["gateway-profile"] },
      { edgeIds: ["profile-cache"] },
      { edgeIds: [], reverseEdgeIds: ["profile-cache", "gateway-profile"] },
    ],
  },
  {
    id: "async-media",
    ...
    weight: 2,
    steps: [
      { edgeIds: ["gateway-media"] },
      { edgeIds: ["media-object"] },
      { edgeIds: ["media-events"] },
      { edgeIds: ["events-media-worker"] },
      { edgeIds: ["worker-object"] },
    ],
  },
  {
    id: "build-deploy-observe",
    scene: "operations",
    weight: 3,
    steps: [
      { edgeIds: ["git-ci", "git-deploy"] },
      { edgeIds: ["ci-registry"] },
      { edgeIds: ["registry-runtime", "deploy-runtime"] },
      { edgeIds: ["runtime-otel"] },
      { edgeIds: ["otel-alert"] },
      { edgeIds: ["alert-email"] },
    ],
  },
  {
    ...
    scene: "operations",
    weight: 1.25,
    steps: [{ edgeIds: ["postgres-backup", "object-backup"] }],
  },
];
...
export function validateSystemDefinition(definition: SystemDefinition): void {
  ...
  assertUnique(
    definition.clusters.map((cluster) => cluster.scene),
    "cluster scene",
  );
  assertUnique(
    definition.standalones.map((entry) => entry.nodeId),
    "standalone node",
  );
  assertUnique(
    definition.nodes.map((entry) => entry.id),
    "node",
  );
  assertUnique(
    definition.edges.map((entry) => entry.id),
    "edge",
  );
  assertUnique(
  ...
  for (const standalone of definition.standalones) {
    maxChars.set(
      standalone.nodeId,
      budgetForSpan(spanForWidth(standalone.width)),
    );
  }
  ...
  for (const entry of definition.nodes) {
    ...
    if (lines.length > 6) {
      throw new Error(
        `Node ${entry.id} has ${lines.length} code lines (max 6)`,
      );
    }
  ...
  for (const cluster of definition.clusters) {
    ...
    for (const placement of cluster.placements) {
      if (!nodeIds.has(placement.nodeId)) {
        throw new Error(
          `Cluster ${cluster.id} references missing node ${placement.nodeId}`,
        );
      }
      if (
        placement.row < 1 ||
        placement.column < 1 ||
        placement.span < 1 ||
        placement.column + placement.span > 7
      ) {
        throw new Error(
          `Cluster ${cluster.id} has an invalid placement for ${placement.nodeId}`,
        );
      }
  ...
  for (const standalone of definition.standalones) {
    if (!nodeIds.has(standalone.nodeId) || !scenes.has(standalone.scene)) {
      throw new Error(
        `Standalone ${standalone.nodeId} has an invalid reference`,
      );
    }
    if (
      standalone.width <= 0 ||
      standalone.width > 100 ||
      standalone.offset < 0
    ) {
      throw new Error(
        `Standalone ${standalone.nodeId} has invalid layout metadata`,
      );
    }
  ...
  for (const entry of definition.edges) {
    ...
    if (!scenes.has(entry.scene)) {
      throw new Error(
        `Edge ${entry.id} references missing scene ${entry.scene}`,
      );
    }
  }
    ...
    for (const step of scenario.steps) {
      for (const edgeId of step.edgeIds) {
        if (!edgeIds.has(edgeId)) {
          throw new Error(
            `Scenario ${scenario.id} references missing edge ${edgeId}`,
          );
        }
      }
      for (const edgeId of step.reverseEdgeIds ?? []) {
        ...
        if (!entry.bidirectional) {
          throw new Error(
            `Scenario ${scenario.id} reverses one-way edge ${edgeId}`,
          );
        }
      }
    }
